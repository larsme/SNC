...
def KittiDepthDataloader(params, sets, mode, device, lidar_padding):
    ...
    gt_filter = params['model']['gt_filter'] if 'gt_filter' in params['model'] else False
    assert not gt_filter or (not lidar_projection and not lidar_padding)

    dataloaders = {}

    if 'train' in sets:
        train_set = KittiDepthDataset(kitti_depth_path=os.path.join(kitti_depth_dir, 'train'), setname='train',
                                      load_rgb=load_rgb, rgb_dir=kitti_rgb_dir, lidar_padding=lidar_padding, lidar_projection=lidar_projection, gt_filter=gt_filter, device=device)
    ...
    if 'selval' in sets:
        test_set = KittiDepthDataset(kitti_depth_path=os.path.join(kitti_depth_dir, 'val_selection_cropped'), setname='selval',
                                     load_rgb=load_rgb, rgb_dir=kitti_rgb_dir, lidar_padding=lidar_paddingg, lidar_projection=lidar_projection, gt_filter=gt_filter, device=device)
    ...
    if 'test' in sets:
        test_set = KittiDepthDataset(kitti_depth_path=os.path.join(kitti_depth_dir, 'test_depth_completion_anonymous'), setname='test',
                                     load_rgb=load_rgb, rgb_dir=kitti_rgb_dir, gt_filter=gt_filter, device=device)
