...
class NConv2d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, pos_fn='softplus', init_method='k', stride=1, padding=0, dilation=1, groups=1, bias=True):
        ...
        super(NConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, (padding,)*4, dilation, False, 0, groups, bias, 'zeros')
...
class NConvCNN(torch.nn.Module):

    def __init__(self, params):
        super().__init__()

        pos_fn = params['pos_fn']
        num_channels = params['n_channels']
